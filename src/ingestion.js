...
const COLUMN_ROLES = {
  ...
  code:        ["item no", "item#", "sku", "code", "cust item no", "item number", "item"],
...
function mapColumnsFromHeader(headerCells) {
  ...
  headerCells.forEach((cell, idx) => {
    ...
    for (const [role, keywords] of Object.entries(COLUMN_ROLES)) {
      if (map[role] !== undefined) continue; // that role already claimed by an earlier column
      if (keywords.some(kw => lower === kw || lower.includes(kw))) {
        map[role] = idx;
        break; // this column's role is decided — don't let it also claim a second role
      }
    }
  });
  return map;
