...
// ── LANDING ───────────────────────────────────────────────────────────
const SERVICES = [
  { key:"ordering", icon:"📋", name:"Smart Ordering", tagline:"Compare vendor prices instantly and build baskets in a couple of taps.",
    detail:"See every vendor's price for the same product side-by-side, automatically ranked cheapest to priciest. Add items to a basket with one tap, and KERDOS tracks your running total against each vendor's delivery minimum so you always know how close you are to a full truck.",
    required:true },
  { key:"import", icon:"📥", name:"Price List Import", tagline:"Drop in a vendor's price list and KERDOS reads it automatically.",
    detail:"Works with Excel, CSV, and PDF — even messy, inconsistently formatted vendor sheets. No manual data entry, no templates to fill out." },
  { key:"invoices", icon:"🧾", name:"Invoice Tracking", tagline:"Every invoice recorded, organized, and IRS-ready.",
    detail:"Upload invoices as they arrive and KERDOS extracts the line items automatically. Search your full spending history any time, sorted by real invoice date." },
  { key:"team", icon:"👥", name:"Team & Roles", tagline:"Bring your team on with the right level of access.",
    detail:"Owners, Managers, and Employees each see exactly what they need. Invite people with a simple code — no email server required." },
];

function LandingGate() {
  const [stage,setStage]=useState("brag"); // brag | services | auth
  const [authMode,setAuthMode]=useState("login");
  const [selected,setSelected]=useState(new Set(["ordering"]));
  const [viewingKey,setViewingKey]=useState(null);

  if(stage==="auth") return <Login initialMode={authMode} onBack={()=>setStage(authMode==="login"?"brag":"services")} selectedServices={[...selected]} />;

  if(stage==="brag") return (
    <div style={{minHeight:"100vh",background:"#003584",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,textAlign:"center"}}>
      ...
      <button onClick={()=>setStage("services")}
        style={{...btn("white","#003584",{padding:"16px 48px",fontSize:16,borderRadius:10,fontWeight:800}),marginBottom:16}}>
        See What KERDOS Can Do →
      </button>
      <button onClick={()=>{setAuthMode("login");setStage("auth");}}
  ...
  // stage === "services"
  const viewing = SERVICES.find(s=>s.key===viewingKey);

  if(viewing) {
    const isSelected = selected.has(viewing.key);
    return (
      <div style={{minHeight:"100vh",background:"#003584",padding:24}}>
        <div style={{maxWidth:520,margin:"0 auto"}}>
          <button onClick={()=>setViewingKey(null)} style={{background:"none",border:"none",color:"rgba(255,255,255,0.8)",fontSize:14,cursor:"pointer",marginBottom:20,fontWeight:600}}>← Back</button>
          <div style={{background:"white",borderRadius:16,padding:32,textAlign:"center"}}>
            <div style={{fontSize:56,marginBottom:14}}>{viewing.icon}</div>
            <div style={{fontWeight:900,fontSize:22,marginBottom:10}}>{viewing.name}</div>
            <div style={{color:"#555",fontSize:14,lineHeight:1.6,marginBottom:26}}>{viewing.detail}</div>
            {viewing.required?(
              <div style={{fontSize:13,fontWeight:700,color:"#0A8A4B"}}>✓ Included with every KERDOS account</div>
            ):(
              <button onClick={()=>{
                setSelected(prev=>{
                  const next=new Set(prev);
                  if(isSelected) next.delete(viewing.key); else next.add(viewing.key);
                  return next;
                });
              }} style={{...btn(isSelected?"#F0F2F5":"#003584",isSelected?"#003584":"white",{padding:"12px 28px",borderRadius:8,fontWeight:800})}}>
                {isSelected?"✓ Added to My Plan":"+ Add to My Plan"}
              </button>
            )}
          </div>
        </div>
  ...
  return (
    <div style={{minHeight:"100vh",background:"#003584",padding:"40px 20px 110px"}}>
      <div style={{maxWidth:760,margin:"0 auto"}}>
        <button onClick={()=>setStage("brag")} style={{background:"none",border:"none",color:"rgba(255,255,255,0.8)",fontSize:14,cursor:"pointer",marginBottom:20,fontWeight:600}}>← Back</button>
        <div style={{textAlign:"center",marginBottom:8}}>
          <div style={{fontWeight:900,fontSize:24,color:"white"}}>Everything KERDOS Does For You</div>
          <div style={{color:"rgba(255,255,255,0.7)",fontSize:14,marginTop:6}}>Pick the tools you want — you can always add more later.</div>
        </div>
        <div style={{display:"flex",gap:14,flexWrap:"wrap",justifyContent:"center",marginTop:28}}>
          {SERVICES.map(s=>{
            const isSelected = selected.has(s.key);
            return (
              <div key={s.key} onClick={()=>setViewingKey(s.key)}
                style={{cursor:"pointer",background:"white",borderRadius:14,padding:"24px 20px",width:210,textAlign:"center",boxShadow:"0 1px 4px rgba(0,0,0,0.15)",border:isSelected?"2px solid #0A8A4B":"1px solid transparent",position:"relative"}}>
                {isSelected&&<div style={{position:"absolute",top:10,right:10,color:"#0A8A4B",fontWeight:900}}>✓</div>}
                <div style={{fontSize:32,marginBottom:10}}>{s.icon}</div>
                <div style={{fontWeight:800,fontSize:15,marginBottom:6}}>{s.name}</div>
                <div style={{color:"#888",fontSize:12,marginBottom:10}}>{s.tagline}</div>
                <div style={{color:"#003584",fontSize:12,fontWeight:700}}>Learn more →</div>
              </div>
            );
          })}
        </div>
      </div>
      <div style={{position:"fixed",bottom:0,left:0,right:0,background:"white",padding:"16px 20px",boxShadow:"0 -2px 10px rgba(0,0,0,0.12)",display:"flex",alignItems:"center",justifyContent:"center",gap:20,flexWrap:"wrap"}}>
        <div style={{fontSize:13,color:"#555",fontWeight:600}}>{selected.size} service{selected.size===1?"":"s"} selected</div>
        <button onClick={()=>{setAuthMode("signup");setStage("auth");}}
          style={{...btn("#003584","white",{padding:"14px 40px",fontSize:15,borderRadius:10,fontWeight:800})}}>
          Get Started →
        </button>
      </div>
    </div>
...
function TeamPanel({orgId,orgName,myRole,currentUserId,onOrgUpdated}) {
    ...
    <div>
      {myRole==="owner"&&(
        <div style={{background:"white",borderRadius:10,padding:16,marginBottom:16,boxShadow:"0 1px 3px rgba(0,0,0,0.06)"}}>
          <div style={{fontSize:11,fontWeight:800,color:"#999",letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:8}}>Organization</div>
          {!editingOrgName?(
        ...
        <div key={m.user_id} style={{background:"white",borderRadius:8,padding:"10px 14px",marginBottom:6,
          ...
          <div style={{minWidth:0,flex:1}}>
            <div style={{fontWeight:700,fontSize:13,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
              {m.email||"Pending — hasn't logged in yet"}{m.user_id===currentUserId?" (you)":""}
            </div>
          </div>
...
function VendorDetail({vendor,vc,vendorItems,invoices,orgId,myRole,onBack,onOpenImport,onOpenRecordInvoice,onUpdated,onEditInvoice,onDeleteInvoice}) {
  ...
  async function doSave(){
    ...
    if(mode==="invoice"){
      const {data:inv,error:invErr}=await supabase.from("invoices").insert({
        ...
        raw_text:text, invoice_date:findDate(text)||new Date().toISOString().split("T")[0],
        invoice_number:findInvoiceNumber(text), status:"recorded",
    ...
    <div style={{fontFamily:"'Inter',-apple-system,sans-serif",minHeight:"100vh",background:"#003584"}}>
      ...
      <div style={{maxWidth:1200,margin:"0 auto",padding:"12px 12px 80px"}}>
        ...
        {tab==="home"&&(
          <div>
            <div style={{textAlign:"center",padding:"36px 16px 28px"}}>
              ...
              <div style={{color:"rgba(255,255,255,0.65)",fontSize:13,marginBottom:22}}>Welcome to KERDOS</div>
              <button onClick={()=>setTab("order")}
                style={{...btn("white","#003584",{padding:"14px 40px",fontSize:15,borderRadius:10,fontWeight:800}),boxShadow:"0 4px 14px rgba(0,0,0,0.2)"}}>
                Get Started →
              </button>
            </div>
            ...
            <div style={{display:"flex",gap:14,flexWrap:"wrap",justifyContent:"center",maxWidth:960,margin:"0 auto"}}>
              ...
              {org.role!=="employee"&&(
                <div onClick={()=>{setSelectedVendorId(null);setImportMode("pricelist");setTab("import");}} style={{cursor:"pointer",background:"white",borderRadius:14,padding:"26px 22px",width:210,textAlign:"center",boxShadow:"0 1px 4px rgba(0,0,0,0.08)",border:"1px solid #EEE"}}>
                  <div style={{fontSize:32,marginBottom:10}}>📥</div>
                  <div style={{fontWeight:800,fontSize:15,marginBottom:4}}>Import Price List</div>
              ...
              {(org.role==="owner"||org.role==="manager")&&(
                <div onClick={()=>setTab("team")} style={{cursor:"pointer",background:"white",borderRadius:14,padding:"26px 22px",width:210,textAlign:"center",boxShadow:"0 1px 4px rgba(0,0,0,0.08)",border:"1px solid #EEE"}}>
                  <div style={{fontSize:32,marginBottom:10}}>👥</div>
                  <div style={{fontWeight:800,fontSize:15,marginBottom:4}}>Team</div>
        ...
        {tab==="vendorDetail"&&vendorDetailId&&(()=>{
          ...
          return (
            <VendorDetail
              vendor={v} vc={vc} vendorItems={vendorItems} invoices={invoices}
        ...
        {tab==="team"&&(org.role==="owner"||org.role==="manager")&&(
          <TeamPanel orgId={org.id} orgName={org.name} myRole={org.role} currentUserId={session.user.id} onOrgUpdated={loadData} />
        )}
      </div>
